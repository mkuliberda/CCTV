...
#TODO: check if it's possible to write file uploads as consumer-producer pattern
# with every upload as separate thread using Threads and Queues



class GoogleDriveImageUploader(AbsUploader.AbstractUploader, AbsObserver.AbstractObserver, PrioMgr.SimplePriorityManager):
    def __init__(self, curl_like_object, image_selector, device_verif_filename,
    ...
    def update(self, value):
        ...
        if value is True and self._is_uploading is not True:
            ...
            current_image = self._image_selector.get_image_relative_path()
            if current_image != self._prev_image:
                print("GDrive update, image: {} uploading...".format(current_image))
                self.upload_image(
                    file_path=current_image,
                    gdrive_folder_id=secrets.get_gdrive_folder_id()
                    )
